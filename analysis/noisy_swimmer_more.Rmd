...
To understand what is happening, let's look at the estimates of
$\pi_1$ for each column of the F matrix. This parameter controls the
weight on the nonzero ("slab") component of the point-exponential
prior.

```{r flash-adapt-prior-2, fig.height=2.5, fig.width=2.5}
pi1 <- sapply(fl_adapt$F_ghat,function (x) x$pi[2])
pdat <- data.frame(k = 1:k,pi1 = pi1)
ggplot(pdat,aes(x = k,y = pi1)) +
  geom_point() +
