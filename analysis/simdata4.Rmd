...
We'll simulate three scenarios, in each of which there are four populations:
  1. One in which vanilla NMF is expected to do well. Inspired by text data: there are four topics, each of which corresponds to one of the four document populations. Each of the four populations is a 70/10/10/10 mixture of dense topics (so the component matrices are not sparse). Documents are all of the same size (so no normalization is necessary).
...
L[, 1] <- c(rep(.7, 250), rep(.1, 750))
L[, 2] <- c(rep(.1, 250), rep(.7, 250), rep(.1, 500))
L[, 3] <- c(rep(.1, 500), rep(.7, 250), rep(.1, 250))
L[, 4] <- c(rep(.1, 750), rep(.7, 250))
...
allplots[[3]] <- plot_nmf(nmf_res, c(6, 1, 5, 4, 2, 3), "NMF (K = 6)")
allplots[[3]]
```
...
allplots[[4]] <- plot_nmf(sparse_nmf, c(9, 6, 4, 3, 1:2, 5, 7:8, 10), "Sparse NMF (correct sparsity)")
allplots[[4]]
```
...
allplots[[5]] <- plot_nmf(sparse_nmf, c(7, 1, 2:6, 8:10), "Sparse NMF (too much sparsity)")
allplots[[5]]
```
...
allplots[[6]] <- plot_nmf(sparse_nmf, c(1, 8, 6, 4, 2, 7, 10, 9, 3, 5), "Sparse NMF (not enough sparsity)")
allplots[[6]]
```
...
allplots[[7]] <- plot_fl(fl, c(2, 4, 3, 5, 1), "EBNMF (greedy)")
allplots[[7]]
```
...
fl_gb <- fl |> 
  ...
  flash_backfit() |>
  flash_nullcheck()
allplots[[8]] <- plot_fl(fl_gb, c(2, 4, 3, 5, 1, 6, 7), "EBNMF (greedy + backfit)") 
allplots[[8]]
```
...
fl_b <- flash_init(Y) |>
  ...
  flash_backfit() |>
  flash_nullcheck()
allplots[[9]] <- plot_fl(fl_b, c(2, 7, 4, 3, 1, 5:6, 8:9), "EBNMF (rank-10 NMF init)") 
allplots[[9]]
```
...
p <- plot_grid(plotlist = allplots, nrow = 3, ncol = 3)
p
```
