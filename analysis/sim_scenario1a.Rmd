...
To begin, let there be four clusters, respectively consisting of 1250, 250, 100, and 10 "pure" documents. We code cluster memberships using a binary $1610 \times 4$ component matrix $L$, with $L_{ik} = 1$ if document $i$ belongs to cluster $k$ and $L_{ik} = 0$ otherwise. Topics are given by a $2000 \times 4$ component matrix $F$. For separability, we include 3 anchor words per topic. For these 12 words, $F_{jk} = 1$ if word $j$ is an anchor word for topic $k$ and $F_{jk} = 0$ otherwise. The remaining $1988 \times 4$ entries $F_{jk}$ are simulated from a Gamma distribution with shape parameter $2$ and scale parameter $1/2$ (i.e., mean $1$ and variance $1/2$), so that all entries $F_{jk}$ are similar in magnitude. We define the matrix of "true means" as $\mu = LF'$. We then simulate the data matrix $Y$ using a `log1p` link function; that is, we simulate count $X_{ij}$ from a Poisson distribution with mean $e^{\mu_{ij}} - 1$, then set $Y_{ij} = \log(X_{ij} + 1)$. (We note that the expectation of the random variable $Y_{ij}$ is not exactly $\mu_{ij}$, but it is close, especially for large $\mu_{ij}$.)
...
```{r nmf_k4}
nmf_k4_res <- run_RcppML_sparse_nmf(Y, k = 4, L1pen = 0, seeds = 1:100)
```
...
for (L1pen in c(.001, .01, .1, 1, 2, 5)) {
  next_res <- run_sparse_nmf(Y, k = 4, L1pen = L1pen)
...
for (L1pen in c(.01, seq(0.1, 0.8, by = 0.1))) {
  next_res <- run_RcppML_sparse_nmf(Y, k = 4, L1pen = L1pen)
  next_p <- plot_nmf(next_res$fit, L, pops, paste0("L1 penalty = ", L1pen))
...
For more flexibility, we can use unimodal nonnegative priors instead of point-exponential priors, which gives the following results:

```{r ebnmf_nmfinit_k4_uni}
ebnmf_nmfinit_k4_uni_res <- run_ebnmf_from_nmf(Ynorm, nmf_norm_k4_res$fit, var_type = 2,
                                               ebnm_fn = ebnm_unimodal_nonnegative)
ebnmf_nmfinit_k4_uni_p <- plot_fl(
  ebnmf_nmfinit_k4_uni_res$fit, L, pops, 
  paste0("Colwise var., Kmax = 4 (ELBO = ", round(ebnmf_nmfinit_k4_uni_res$fit$elbo, 2), ")")
)

ebnmf_nmfinit_k8_uni_res <- run_ebnmf_from_nmf(Ynorm, nmf_norm_k8_res$fit, var_type = 2,
                                               ebnm_fn = ebnm_unimodal_nonnegative)
ebnmf_nmfinit_k8_uni_p <- plot_fl(
  ebnmf_nmfinit_k8_uni_res$fit, L, pops, 
  paste0("Colwise var., Kmax = 8 (ELBO = ", round(ebnmf_nmfinit_k8_uni_res$fit$elbo, 2), ")")
)

ebnmf_nmfinit_k4_vt0_uni_res <- run_ebnmf_from_nmf(Ynorm, nmf_norm_k4_res$fit, var_type = 0,
                                               ebnm_fn = ebnm_unimodal_nonnegative)
ebnmf_nmfinit_k4_vt0_uni_p <- plot_fl(
  ebnmf_nmfinit_k4_vt0_uni_res$fit, L, pops, 
  paste0("Const var., Kmax = 4 (ELBO = ", round(ebnmf_nmfinit_k4_vt0_uni_res$fit$elbo, 2), ")")
)

ebnmf_nmfinit_k8_vt0_uni_res <- run_ebnmf_from_nmf(Ynorm, nmf_norm_k8_res$fit, var_type = 0,
                                               ebnm_fn = ebnm_unimodal_nonnegative)
ebnmf_nmfinit_k8_vt0_uni_p <- plot_fl(
  ebnmf_nmfinit_k8_vt0_uni_res$fit, L, pops, 
  paste0("Const var., Kmax = 8 (ELBO = ", round(ebnmf_nmfinit_k8_vt0_uni_res$fit$elbo, 2), ")")
)

plot_grid(ebnmf_nmfinit_k4_vt0_uni_p, ebnmf_nmfinit_k8_vt0_uni_p,
          ebnmf_nmfinit_k4_uni_p, ebnmf_nmfinit_k8_uni_p, 
          nrow = 2, ncol = 2)
```

With Kmax = 8, unimodal nonnegative priors give relatively poor results.
...
We can also compare with results that use unimodal nonnegative priors (I couldn't get the strictly alternating approach to work with Kmax = 8, so it is omitted here):

```{r ebnmf_other_uni}
fl_g_uni <- flash(Ynorm, var_type = 2, greedy_Kmax = 8, ebnm_fn = ebnm_unimodal_nonnegative, verbose = 0)
fl_b_uni <- fl_g_uni |> flash_backfit(verbose = 0) 
p_g_uni <- plot_fl(fl_g_uni, L, pops, paste0("Greedy (ELBO = ", round(fl_g_uni$elbo, 2), ")"))
p_b_uni <- plot_fl(fl_b_uni, L, pops, paste0("Backfit (ELBO = ", round(fl_b_uni$elbo, 2), ")"))

fl_bulkalt_g_uni <- fl_b_uni |> flash_greedy(Kmax = 4, ebnm_fn = ebnm_unimodal_nonnegative, verbose = 0)
fl_bulkalt_b_uni <- fl_bulkalt_g_uni |> flash_backfit(verbose = 0)
p_bulkalt_g_uni <- plot_fl(fl_bulkalt_g_uni, L, pops, 
                           paste0("Second greedy (ELBO = ", round(fl_bulkalt_g_uni$elbo, 2), ")"))
p_bulkalt_b_uni <- plot_fl(fl_bulkalt_b_uni, L, pops, 
                           paste0("Second backfit (ELBO = ", round(fl_bulkalt_b_uni$elbo, 2), ")"))

plot_grid(p_g_uni, p_b_uni, p_bulkalt_g_uni, p_bulkalt_b_uni, nrow = 2, ncol = 2)
```
