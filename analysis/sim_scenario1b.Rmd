...
run_sims <- function(which_dat, Kmax, verbose = FALSE) {
  ...
  for (varied_n in c(3, 5, 10, 25, 50)) { 
    for (disp in c(0.1, 1, 10, 100, 10000)) {
      ...
      # all_res <- all_res |>
      #   bind_rows(next_tib(next_seed, disp, varied_n, "NMF-EBNMF-uni", Kmax, ebnmf_nmf_res, sim_dat))
      ...
      # all_res <- all_res |>
      #   bind_rows(next_tib(next_seed, disp, varied_n, "EBNMFgb-uni", Kmax, ebnmf_gb_uni_res, sim_dat))
