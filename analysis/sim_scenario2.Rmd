...
calc_metrics <- function(res, sim_dat) {
  ...
  # Metrics for 4-6 are the scales of these (redundant/noisy) components:
...
set.seed(1)
for (shared_n in c(25, 100, 500, 900, 975)) {
  for (shape in c(1/2, 2/3, 3/4, 1, 2)) {
    cat("SHARED N: ", shared_n, "SHAPE: ", shape, "\n")
    
    gamma_mean <- 1
    scale <- gamma_mean / shape 
    
    ns <- c(rep(1000 - shared_n, 3), rep(shared_n, 3))
    p <- 1000
...
ggplot(plot_df, aes(x = shared_n, y = shape, fill = metric_val)) +
  ...
  scale_fill_gradient(low = low_color, high = high_color, na.value = na_color, transform = "logit",
                      breaks = c(0.9, 0.99, 0.999, 0.9999)) +
  facet_grid(rows = vars(metric_type), cols = vars(method), scales = "free_x") +
  labs(x = "Size of shared populations", y = "Shape of gamma prior on factors", fill = "Cor. w/ true F") +
  theme(axis.text.x = element_text(angle = 45))
```
...
ggplot(plot_df, aes(x = shared_n, y = shape, fill = metric_val)) +
  ...
  facet_grid(rows = vars(metric_type), cols = vars(method), scales = "free_x") +
  labs(x = "Size of shared populations", y = "Shape of gamma prior on factors", fill = "Scale") +
  theme(axis.text.x = element_text(angle = 45))
```
...
ggplot(plot_df, aes(x = shared_n, y = shape, fill = metric_val)) +
  ...
  labs(x = "Size of shared populations", y = "Shape of gamma prior on factors", fill = "Time elapsed (s)") +
  theme(axis.text.x = element_text(angle = 45))
```
