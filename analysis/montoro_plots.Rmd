...
fitted_priors_plot <- function (ghat) {
  ...
  p1 <- ggplot(pdat1,aes(x = pi,y = k)) +
    geom_point() +
	labs(x = "prior probability") +
    theme_cowplot(font_size = 9)
  p2 <- ggplot(pdat2,aes(x = scale,y = k)) +
    geom_point() +
...
p3 <- structure_plot(F[rows1,],grouping = celltypes[rows1],gap = 10,
                     topics = factors_to_plot,colors = factor_colors) +
  labs(y = "membership",fill = "factor",title = "30 days") +
  theme(plot.title = element_text(size = 9,face = "plain"),
...
fitted_priors_plot(fl_k40$L_ghat)
```

ADD TEXT HERE

```{r point-exp-priors-cells-k40, fig.height=4, fig.width=3.5}
fitted_priors_plot(fl_k40$F_ghat)
```
