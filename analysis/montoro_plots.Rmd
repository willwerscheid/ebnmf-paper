...
```{r residuals-k12, fig.height=2.5, fig.width=2.65}
...
```{r residuals-k30, fig.height=2.5, fig.width=2.65}
...
```{r point-exp-priors-k30-ggsave, echo=FALSE}
ggsave("montoro_fitted_priors_k30.pdf",
...
```{r another-structure-plot-k30-ggsave, echo=FALSE}
ggsave("montoro_structure_plot_k30_alldays.pdf",p,height = 2.75,width = 8)
```
...
key_genes <- c("Krt5","Dapl1",     # Basal (k2, k4, k9)
			   ...
			   "Krt18","Sfn",      # Transitional/differentiating (k2)
...
Now examine expression of some cell-cycle genes factorss the factors:

```{r cell-cycle-genes-k30}
top_cell_cycle <- c("Cdk1","Ube2c","Top2a")
round(L[top_cell_cycle,],digits = 3)
```

Save the LFC estimates from the $K = 30$ fit to a CSV file:

```{r write-lfc-k30}
out <- as.data.frame(round(L,digits = 4))
out <- cbind(data.frame(gene = rownames(out)),out)
write.csv(format(out,scientific = FALSE,digits = 4),
          file = "montoro_lfc_k30.csv",quote = FALSE,row.names = FALSE)
```
