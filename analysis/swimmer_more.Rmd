...
faactors—by "vanilla", we meaan NMF without any penalties on the
parameters. See the
...
This is perhaps a more "fair" comparison to NMF since both flashier
and NMF were initialized in the same way. (And the greedy
initialization is particularly well suited for the swimmer data set
because it identifies the torso as the first factor.)

Let's see if we can improve the decomposition further by adapting the
priors:

```{r flash-adapt-rand-init, results="hide", fig.height=3, fig.width=7}
L0 <- ldf(fit2)$L
F0 <- ldf(fit2)$F
...
This improved the decomposition slightly by "cleaning up" the messy
factor, factor 16.
