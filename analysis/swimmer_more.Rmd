...
The greedy initialization appears particularly well suited for the swimmer data set (backfitting only needs to clean up the first factor). So let's
...
L0 <- ldf(fit3)$L
F0 <- ldf(fit3)$F
fit5 <- flash_init(Y,var_type = 0)
fit5 <- flash_factors_init(fit5,list(L0,F0),ebnm_point_exponential)
fit5 <- flash_backfit(fit5)
plot_images(ldf(fit5)$L,nrow = 3,ncol = 8)
```
