...
allplots[[2]] <- plot_nmf(nmf_res, c(1, 3, 4, 2), "NMF")
allplots[[2]]
```
...
allplots[[3]] <- plot_nmf(nmf_res, c(4, 8, 3, 6, 2, 5, 1, 7), "NMF")
allplots[[3]]
```
...
fl_b <- flash_init(Y) |>
  ...
  flash_nullcheck()
allplots[[4]] <- plot_fl(fl_b, c(4, 7, 3, 6, 1, 2, 5), "EBNMF (NMF init)") 
allplots[[4]]
```
...
allplots[[5]] <- plot_fl(fl, c(2, 4, 3, 1), "EBNMF (greedy)")
allplots[[5]]
```
...
fl_gb4 <- fl |> 
  flash_backfit() 
allplots[[6]] <- plot_fl(fl_gb4, c(2, 4, 3, 1), "EBNMF (backfit)") 
allplots[[6]]
```
...
fl_gb8 <- fl |> 
  flash_backfit() 
allplots[[8]] <- plot_fl(fl_gb8, c(2, 4, 3, 5, 1), "EBNMF (backfit)") 
allplots[[8]]
```
...
allplots[[10]] <- plot_nmf(sparse_nmf, c(4, 1, 2, 3), "SpNMF (good sparsity)")
allplots[[10]]
```
...
allplots[[9]] <- plot_nmf(sparse_nmf, c(4, 3, 2, 1), "SpNMF (low sparsity)")
allplots[[9]]
```
...
allplots[[11]] <- plot_nmf(sparse_nmf, c(4, 1, 2, 3), "SpNMF (high sparsity)")
allplots[[11]]
```
...
allplots[[12]] <- plot_nmf(sparse_nmf, c(6, 2, 1, 8, 5, 4, 3, 7), "SpNMF (good sparsity)")
allplots[[12]]
```
