...
In this analysis we consider the "fully-separated topics" scenario. Here there are four populations of varying abundance; the most abundant population is of size 700, while the rarest population ranges from 3 to 50 individuals. The `L` matrix codes population memberships. The factors `F` are randomly generated from a Gamma distribution with mean 1, where the shape of the distribution ranges from 0.25 (so that counts vary widely across columns) to 4 (similar counts across columns). For separability in the `K = 4` case, we create 10 "anchor words" for each topic. Finally, we use a log1p link function in the sense that we simulate counts $X$ as Poisson($e^\mu - 1$), where $\mu = LF'$, and then take the data matrix to be $Y = \log(X + 1)$. 
...
sim_data <- function(ns, p, gamma_shape, gamma_scale, n_anchor_words = 10, link = "log1p") {
  ...
  set.seed(1)
  F <- matrix(rgamma(p * 4, shape = gamma_shape, scale = gamma_scale), nrow = p, ncol = 4)
...
calc_metrics <- function(res, sim_dat) {
  ...
  # Metrics for components 1-4 are correlations:
  ...
  # Metrics for 5-8 are the scales of these (redundant/noisy) components:
  ...
  unmatched_scales <- c(unmatched_scales, rep(NA, 4 - length(unmatched_scales)))
...
set.seed(1)
for (rare_n in c(3, 5, 10, 15, 20, 30, 50)) {
  for (shape in c(0.25, 0.33, 0.5, 0.75, 1, 2, 4)) {
    cat("RARE N: ", rare_n, "SHAPE: ", shape, "\n")
    
    gamma_mean <- 1
    scale <- gamma_mean / shape 
    
    ns <- c(700 - rare_n, 250, 50, rare_n)
    p <- 500
    ...
    all_res <- all_res |> 
      bind_rows(next_tib(shape, ns, "NMF", 4, nmf_res_k4, sim_dat))
    
    all_res <- all_res |> 
      bind_rows(next_tib(shape, ns, "NMF-EBNMF0", 4, run_ebnmf_from_nmf(Y, nmf_res_k4$fit, var_type = 0), sim_dat))
    
    all_res <- all_res |> 
      bind_rows(next_tib(shape, ns, "NMF-EBNMF2", 4, run_ebnmf_from_nmf(Y, nmf_res_k4$fit, var_type = 2), sim_dat))

    all_res <- all_res |> 
      bind_rows(next_tib(shape, ns, "EBNMF0", 4, run_alternating_gb(Y, Kmax = 4, var_type = 0), sim_dat))

    all_res <- all_res |> 
      bind_rows(next_tib(shape, ns, "EBNMF2", 4, run_alternating_gb(Y, Kmax = 4, var_type = 2), sim_dat))
    
    nmf_res_k8 <- run_nmf(Y, k = 8)
    all_res <- all_res |> 
      bind_rows(next_tib(shape, ns, "NMF", 8, nmf_res_k8, sim_dat))
    
    all_res <- all_res |> 
      bind_rows(next_tib(shape, ns, "NMF-EBNMF0", 8, run_ebnmf_from_nmf(Y, nmf_res_k8$fit, var_type = 0), sim_dat))
    
    all_res <- all_res |> 
      bind_rows(next_tib(shape, ns, "NMF-EBNMF2", 8, run_ebnmf_from_nmf(Y, nmf_res_k8$fit, var_type = 2), sim_dat))

    all_res <- all_res |> 
      bind_rows(next_tib(shape, ns, "EBNMF0", 8, run_alternating_gb(Y, Kmax = 8, var_type = 0), sim_dat))

    all_res <- all_res |> 
      bind_rows(next_tib(shape, ns, "EBNMF2", 8, run_alternating_gb(Y, Kmax = 8, var_type = 2), sim_dat))
  }
}
```
...
```{r}
plot_df <- all_res |> filter(Kmax == 4, str_starts(metric_type, "LLcor")) |>
  mutate(rare_n = factor(rare_n), 
         shape = factor(shape),
         method = factor(method, levels = c("NMF", "NMF-EBNMF2", "NMF-EBNMF0", "EBNMF2", "EBNMF0"))) |>
  mutate(metric_val = pmin(metric_val, 0.9999))
ggplot(plot_df, aes(x = rare_n, y = shape, fill = metric_val)) +
  geom_tile() +
  scale_fill_gradient(low = "red", high = "white", na.value = "red", transform = "logit") +
...
```{r}
plot_df <- all_res |> filter(Kmax == 4, str_starts(metric_type, "FFcor")) |>
  mutate(rare_n = factor(rare_n), 
         shape = factor(shape),
         method = factor(method, levels = c("NMF", "NMF-EBNMF2", "NMF-EBNMF0", "EBNMF2", "EBNMF0"))) |>
  mutate(metric_val = pmin(metric_val, 0.9999))
...
```{r}
plot_df <- all_res |> filter(Kmax == 8, str_starts(metric_type, "LLcor")) |>
  mutate(rare_n = factor(rare_n), 
         shape = factor(shape),
         method = factor(method, levels = c("NMF", "NMF-EBNMF2", "NMF-EBNMF0", "EBNMF2", "EBNMF0"))) |>
  mutate(metric_val = pmin(metric_val, 0.9999))
...
```{r}
plot_df <- all_res |> filter(Kmax == 8, str_starts(metric_type, "FFcor")) |>
  mutate(rare_n = factor(rare_n), 
         shape = factor(shape),
         method = factor(method, levels = c("NMF", "NMF-EBNMF2", "NMF-EBNMF0", "EBNMF2", "EBNMF0"))) |>
  mutate(metric_val = pmin(metric_val, 0.9999))
...
Scale of redundant/noisy factors:

```{r}
plot_df <- all_res |> filter(Kmax == 8, str_starts(metric_type, "Scale")) |>
  mutate(rare_n = factor(rare_n), 
         shape = factor(shape),
         method = factor(method, levels = c("NMF", "NMF-EBNMF2", "NMF-EBNMF0", "EBNMF2", "EBNMF0")))
ggplot(plot_df, aes(x = rare_n, y = shape, fill = metric_val)) +
